...
public class CommandArguments {
    ...
    public static ArgumentNodeBuilder<Enchantment> enchantArgument(String name) {
        return Arguments.enchantment(name).suggestions((reader, context) -> BukkitThing.getAsStrings(
            RegistryType.ENCHANTMENT));
    }


    public static ArgumentNodeBuilder<CustomEnchantment> customEnchantArgument(String name) {
        return Commands.argument(name, (context, string) -> Optional.ofNullable(EnchantRegistry.getById(string))
            .orElseThrow(() -> CommandSyntaxException.custom(CoreLang.COMMAND_SYNTAX_INVALID_ENCHANTMENT))
        )
    ...
    public static ArgumentNodeBuilder<Integer> levelArgument(String name) {
    ...
    public static ArgumentNodeBuilder<EquipmentSlot> slotArgument(String name) {
        return Commands.argument(name, (context, string) -> Enums.parse(string, EquipmentSlot.class)
            ...
            .suggestions((reader, context) -> Arrays.stream(EntityUtil.EQUIPMENT_SLOTS).map(Enum::name).map(
